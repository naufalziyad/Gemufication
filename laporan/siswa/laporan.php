...
?>
		...
		<div class="container">
 		...
 		<form class="form-search" align="right" method="GET" action="">
		  <div class="input-append">
			...
			<?php
					$query_input_ujian = mysql_query("select ujian.id,ujian.nama_ujian from ujian,hasil_ujian where hasil_ujian.id_ujian=ujian.id and hasil_ujian.id_siswa=$id_siswa");
 ...
 for($n=0;$n<=20;$n++){
	...
	$query_nilai = mysql_query("SELECT COUNT(*) AS jml_data FROM hasil_ujian,siswa,kelas where hasil_ujian.id_siswa=siswa.id and siswa.id_kelas=kelas.id and siswa.id=$id_siswa and hasil_ujian.skor >= $tingkat_nilai and hasil_ujian.skor < $tingkat_nilai2 $txt_ujian $txt_kelas");
 ...
 <!--=====================================================================================================================-->
		...
		<?php
			...
			$query_page = mysql_query("select $select_hasil_ujian $select_siswa $select_ujian $select_kelas $select_jurusan from hasil_ujian,ujian,siswa,kelas,jurusan where hasil_ujian.id_siswa=siswa.id and hasil_ujian.id_ujian=ujian.id and siswa.id_kelas=kelas.id and siswa.id=$id_siswa and kelas.id_jurusan=jurusan.id $txt_ujian $txt_kelas order by tgl desc LIMIT $offset, $dataPerPage");
		...
		<div class="pagination pagination-centered">
            <ul>
                <li><a href="?page=Laporan&halaman=1&ujian=<?php echo $page_ujian; ?>">First</a></li>
                <li>
					<?php
					if ($noPage > 1) echo  "<a href='?page=Laporan&halaman=".($noPage-1)."&ujian=".$page_ujian."&kelas=".$page_kelas."'>Prev</a>";
                   ?>
                </li>
					<?php
						$noPage1 = $noPage - 1;
						...
						$noPage3 = $noPage + 1;
						$noPage4 = $noPage + 2;
						if($noPage > 2)echo "<li><a href='?page=Laporan&halaman=".$noPage2."&ujian=".$page_ujian."'>".$noPage2."</a></li>";
						if($noPage > 1)echo "<li><a href='?page=Laporan&halaman=".$noPage1."&ujian=".$page_ujian."'>".$noPage1."</a></li>";
						echo "<li class='active'><a href='?page=Laporan&halaman=".$noPage."&ujian=".$page_ujian."'>".$noPage."</a></li>";
						if($jumPage > $noPage){
							echo "<li><a href='?page=Laporan&halaman=".$noPage3."&ujian=".$page_ujian."'>".$noPage3."</a></li>";
							if($jumPage > $noPage3)echo "<li><a href='?page=Laporan&halaman=".$noPage4."&ujian=".$page_ujian."'>".$noPage4."</a></li>";
						}
					
					?>
                <li>
                        <?php
						if ($noPage < $jumPage) echo "<li><a href='?page=Laporan&halaman=".($noPage+1)."&ujian=".$page_ujian."'>Next</a>";
						?>
                </li>
                <li><a href="?page=Laporan&halaman=<?php echo $jumPage; ?>&ujian=<?php echo $page_ujian; ?>">Last(<?php echo $jumPage; ?>)</a></li>
            </ul>
        </div>
 <!--=========================================================================================================================-->
