...
$id     = $_GET["id"];
		...
		$cek_query_kelas = mysql_query("select*from kelas where id_jurusan='$id'");
		while($cek_array_kelas = mysql_fetch_array($cek_query_kelas))
		{
			$id_kelas = $cek_array_kelas[id];
			$cek_query_siswa = mysql_query("select*from siswa where id_kelas='$id_kelas'");
			$cek_query_matpel = mysql_query("select*from matpel where id_kelas='$id_kelas'");
		...
		$set = mysql_query("delete from jurusan where id='$id'");
			if($set){
				$form1  = "<form method='POST' action='../../' id='alert'>";
				$form1 .= "<input type='hidden' name='success' value='delete'>";
				$form1 .= "<input type='hidden' name='page' value='Server'>";
				$form1 .= "</form>";
