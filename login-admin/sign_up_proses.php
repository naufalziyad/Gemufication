...
$kelas    = $_POST["kelas"];
$email    = $_POST["email"];
if(empty($nis) || empty($nama) || empty($password) || empty($kelas) || empty($email)){
	...
	if($nis != $cek_nis){
		mysql_query("insert into siswa(nis,nama_siswa,password,id_kelas,email) values('$nis','$nama','$password','$kelas','$email')");
